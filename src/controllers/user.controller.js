...
const getChannelProfileInfo = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    {
      // we are trying to find our user here with username match operator is used for it
      $match: {
        username: username,
      },
    },
    {
      // we are joining the subscription doc with the user's doc based on the channels so we can get number of subscribers
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    {
      // we are joining subsription doc with user found in match to find the channels he has subscribed
      $lookup: {
        from: "subscriptions",
        localField: "_id",
    ...
    {
      // we are adding more fields to user doc with add field the fields that i found with lookup
    ...
    {
      // project here is used to send the data we want to send and what we not to next stage
      $project: {
        fullname: 1,
        username: 1,
        email: 1,
  ...
  if (!channel?.length) {
    throw new ApiError(400, "channel not found aggergation failed");
  }

  return res
    .status(200)
    .json(new ApiResponse(200, channel[0], "user info fetched successfully"));
});

// get user's watchHistory
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        user[0].watchHistory,
        "user watch History fetched succesfully"
      )
    );
});
